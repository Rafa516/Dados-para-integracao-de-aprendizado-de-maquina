...
if not os.path.exists('modelo_evasao_escolar.pkl'):
    ...
    if st.sidebar.button("Carregar Dados da API"):
        ...
        if student_data_list:
            # Processar cada estudante retornado pela API
            for student_data in student_data_list:
                ...
                # Exibir o resultado
                st.write("### Resultado da Previsão para o Estudante")
                st.write(f"Idade: {idade}, Frequência: {frequencia}%, Nota Média: {nota_media}, Distância da Escola: {distancia_escola} km, Situação Socioeconômica: {situacao_socioeconomica}")
                if prediction[0] == 1:
                ...
                st.write("---")  # Separador entre os resultados dos estudantes